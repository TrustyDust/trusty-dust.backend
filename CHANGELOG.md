...
- Added rate limiting via `@nestjs/throttler` for `/auth/login` (5 req/min), social interactions (20–60 req/min tiers), wallet reputation analysis (10 req/5 min), `/zk/generate` (5 req/min), jobs create/apply/submit/confirm (10–30 req/5 min), notification polling (60 req/min), chat endpoints (conversation list/create/messages at 60/20/60 req/min), and trust/tier reads (120 req/min) to prevent abuse.
